...
int n;
int ans1,ans2,ans3 = INF; // 1/ans1,2/(2ans2+1),ans3
...
void dfs_ans(int u,int p){
	...
	if(a[u] == 1){
		remax(ans1,dp[u]);
		remax(ans1,1+dp_par[u]);
		remax(ans1,1+mx1+mx2);
	}
	if(mx1 == mx2 and a[u] == 2) remax(ans2,mx1);
	for(auto v:adj[u]){
		if(v != p) dfs_ans(v,u);
	}
}
...
void solvethetestcase(){
    ...
    dfs_par(1,1);
    dfs_ans(1,1);
    if(ans3 != 1) cout << ans3 << "/1\n";
    else if(ans2 == ans1) cout << "2/" << 2*ans2+1 << "\n";
    else cout << "1/" << ans1 << "\n";
}