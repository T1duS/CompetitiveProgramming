/*
Soln: https://csacademy.com/contest/ioi-2016-training-round-2/task/increasing_subarrays/solution/
*/
...
int n,m;
int a[MAXN],pref[MAXN],opt[MAXN],val[MAXN],nxt[MAXN];
// int seg[MAXN];
...
void solvethetestcase(){
    ...
    FORD(i,n-1,1){
        ...
        while(dq.size() and a[dq.front()] <= a[i]){
            ...
            u = dq.front();
            dq.pop_front();
            if(dq.size()) v = dq.front();
            else v = r+1;
            val += (v-u)*(a[i]-a[u]);
            // trace(i,u,v,pref[v-1]-pref[u-1],a[i]*(v-u));
            // trace(val);
        }
        dq.push_front(i);
        int mx = a[dq.back()];
        while(val > m){
            // trace(val,r,mx,a[r],dq.back());
            if(dq.back() == r){
                dq.pop_back();
                mx = a[dq.back()];
            } 
